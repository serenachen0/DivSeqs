#!/usr/bin/env python3
"""
(1) Dynamic filtering and selection of sequence homologs using pairwise sequence identity or sequence coverage cutoffs
(2) Generate a histogram of number of unique homologs in each pairwise sequence identity cutoff bin

Authors: Serena Chen <chens@ornl.gov>
Last edit: May 5, 2025
"""
...
def create_parser():
    parser = argparse.ArgumentParser(
        description="Dynamic filtering and selection of sequence homologs from an MSA in A3M format using HHfilter"
    )
    parser.add_argument(
        "hhfilter",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=int(50),
        help="Minimum coverage with the first (query) sequence in the input MSA file; default = 50",
    )
    parser.add_argument(
        "--idmin",
        type=int,
        default=int(0),
        help="Minimum pairwise sequence identity cutoff in dynamic filtering; default = 0",
    )
    parser.add_argument(
        "--idmax",
        type=int,
        default=int(100),
        help="Maximum pairwise sequence identity cutoff in dynamic filtering; default = 100",
    )
    parser.add_argument(
        "--outdir",
        type=str,
        default='.',
        help="Path to the output file directory; default = current directory"
    )
    parser.add_argument(
        "--pid",
        type=str,
        default='MyProtein',
        help="Protein ID to be used for output file name and plot; default = MyProtein"
    )
...
def run(args):
    ...
    seqs_list = []
    unique_list = []
    for iden in range(id_min, id_max+1, id_stride):
        #run HHfilter
        tmp = outdir + "/tmp.a3m"
    ...
    ind = np.arange(id_min, id_max+1, id_stride) # the x locations for the groups
    ...
    plt.xticks(np.arange(id_min, id_max+1, 10))
